...
namespace InstaAPI.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        [HttpGet("login")]
        public async Task<IActionResult> Login(LoginReq user)
        {
            if (!uow.UserRepository.GetListAsync().Result.Any(u => u.Email == user.Email))
            {
                return BadRequest("User doesn't exist.");
            }
            IEnumerable<User> results = await uow.UserRepository.GetListAsync();
            User result = results.Where(u => u.Email == user.Email).FirstOrDefault();
            return Ok(new
            {
